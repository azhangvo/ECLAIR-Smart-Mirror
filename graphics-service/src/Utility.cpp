...
{
    // draw rectangle for the button
    DrawRectangle(x, y, width, height, SKYBLUE);

    // draw text for the button
    DrawText(text.c_str(), x + 100, y + 150, height / 4, DARKBLUE);
    ...
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
    {
        ...
        if (x <= curMouseX && curMouseX <= x + width)
        {
            if (y <= curMouseY && curMouseY <= y + height)
            {
                cout << "test";
            }
        }
